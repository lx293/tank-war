...
public class GamePanel extends JPanel {
    ...
    private void handleInput() {
        ...
        if (canMove) {
            for (Bot bot : enemies) {
                if (bot.alive && newRect.intersects(bot.getRect())) {
                    canMove = false;
                    break;
                }
            }
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Wall w : walls) {
            ...
            if (w.canBreak) {
                ...
                int halfW = sw / 2;
                int halfH = sh / 2;
                g2d.fillRect(sx, sy, halfW, halfH);
                g2d.fillRect(sx + halfW, sy + halfH, halfW, halfH);
                g2d.setColor(new Color(100, 50, 0));
                g2d.drawRect(sx, sy, sw, sh);
                g2d.drawLine(sx + halfW, sy, sx + halfW, sy + sh);
                g2d.drawLine(sx, sy + halfH, sx + sw, sy + halfH);
            } else {
                ...
                g2d.drawLine(sx, sy, sx + sw, sy);
                g2d.drawLine(sx, sy, sx, sy + sh);
            }
        }

        if (player.alive) {
            drawTank(g2d, player.x, player.y, player.dir, new Color(50, 180, 50), true, scaleX, scaleY, scale);
        }

        for (Bot e : enemies) {
            if (!e.alive) continue;
            drawTank(g2d, e.x, e.y, e.dir, new Color(200, 50, 50), false, scaleX, scaleY, scale);
        }
    ...
    private void drawTank(Graphics2D g2d, int x, int y, int dir, Color bodyColor, boolean isPlayer, double scaleX, double scaleY, double scale) {
        int sx = (int) (x * scaleX);
        int sy = (int) (y * scaleY);
        int s = (int) (40 * scale);
        int cx = sx + s / 2;
        ...
        if (dir == 0 || dir == 2) {
            g2d.fillRect(sx, sy, (int)(8*scale), s);
            g2d.fillRect(sx + s - (int)(8*scale), sy, (int)(8*scale), s);
        } else {
            g2d.fillRect(sx, sy, s, (int)(8*scale));
            g2d.fillRect(sx, sy + s - (int)(8*scale), s, (int)(8*scale));
        }

        g2d.setColor(bodyColor);
        g2d.fillRect(sx + (int)(8*scale), sy + (int)(8*scale), s - (int)(16*scale), s - (int)(16*scale));

        g2d.setColor(isPlayer ? Color.RED : new Color(50, 180, 50));
        int bw = (int)(6*scale);
        int bl = (int)(20*scale);
        switch (dir) {
            case 0: g2d.fillRect(cx - bw/2, sy, bw, bl); break;
            case 1: g2d.fillRect(sx + s - bl, cy - bw/2, bl, bw); break;
            case 2: g2d.fillRect(cx - bw/2, sy + s - bl, bw, bl); break;
            case 3: g2d.fillRect(sx, cy - bw/2, bl, bw); break;
        }
    }
}